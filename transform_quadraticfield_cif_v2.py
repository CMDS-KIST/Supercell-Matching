...
default_work_dir = "transform_quadfield"

def get_transformed_quadfield_structure(initial_structure : Structure, m1, m2, d):
    transformed_structure = initial_structure.copy()
    if d % 4 != 1:
        transformed_structure.make_supercell([[m1, m2, 0], [m2*d, m1, 0], [0, 0, 1]])
    elif d % 4 == 1:
        transformed_structure.make_supercell([[m1, m2, 0], [m2*(d - 1)//4, m1 - m2, 0], [0, 0, 1]])
...
def get_transformed_quadfield_surface_for_heterostructure(initial_structure, m1, m2, d, is_top = True):
    transformed_structure = initial_structure.copy()
    if d % 4 != 1:
        transformed_structure.make_supercell([[m1, m2, 0], [m2*d, m1, 0], [0, 0, 1]])
    elif d % 4 == 1:
...
def export_transformed_quadfield_surface_for_heterostructure(initial_structure_filename, m1, m2, d, is_top = True, work_dir = default_work_dir, export_type = 'cif'):
    ...
    transformed_structure = get_transformed_quadfield_surface_for_heterostructure(initial_structure, m1, m2, d, is_top)
...
def export_heterostructure(top_structure_filename, m1, m2, bottom_structure_filename, n1, n2, d, work_dir = default_work_dir):
    if not os.path.isdir(work_dir):
        os.mkdir(work_dir)

    top_transformed = export_transformed_quadfield_surface_for_heterostructure(top_structure_filename, m1, m2, d, work_dir=work_dir)
    bottom_transformed = export_transformed_quadfield_surface_for_heterostructure(bottom_structure_filename, n1, n2, d, is_top=False, work_dir=work_dir)
...
def main():
    ...
    export_heterostructure(args.Top_cif_filename, args.m1, args.m2, args.Bottom_cif_filename, args.n1, args.n2, args.d)
